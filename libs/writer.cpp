...
bool Writer::WriteFloatLE(float dword){
    static_assert(sizeof(dword) == 4, "Float size error");
    if (!this->UpdateState(4)) {
        return false;
    } else {
        uint8_t * float_as_char = reinterpret_cast<uint8_t *>(&dword);

        this->_buffer[this->_position] = float_as_char[0];
        this->_buffer[this->_position + 1] = float_as_char[1];
        this->_buffer[this->_position + 2] = float_as_char[2];
        this->_buffer[this->_position + 3] = float_as_char[3];
        this->_position += 4;
        return true;
    }
}
